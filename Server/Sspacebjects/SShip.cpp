...
void SShip::proces(){
	...
	this->addRecoil(_sType->getRecoilRecharge());
	...
	if (_targetUpdateCounter % 25 == 0){
		...
		this->_recharge = _sType->getRecharge() * 1000;
		this->_maxEnergy = _sType->getEnergy() * 1000;
		this->_maxdeflector = _sType->getDeflector()*1000;
		//this->_deflector = stype.getDeflector()*1000;
		for (int i = 0; i< 6; i++){
			this->_maxshield[i] = _sType->getShield(i) * _sType->getShieldStr() * 10;
		}
		this->_maxarmor = _sType->getArmor()*1000;
		this->_maxhull = _sType->getHull()*1000;

		this->_scanRange = _sType->getScanRange() * 100;
		this->_scanPRange = _sType->getScanPRange() * 100;
		_topSpeed = _sType->getTopSpeed() * 100;
		_agility = _sType->getAgility();
		for (map<BonusTypes::Enum, int32_t>::iterator it = this->_bonuslist.begin(); it != this->_bonuslist.end();it++){
			switch(it->first){
				...
				case BonusTypes::SpeedThruster:{this->_topSpeed+= (it->second/(_sType->getMass()/10));break;}
				case BonusTypes::ManuvereThruster:{this->_agility+= (it->second/(_sType->getMass()*10));break;}			
			}
		}
		for(SSlotNodeI it = this->slots.begin(); it != this->slots.end(); it++){
			if (it->second->getSS())
	...
	for (list<uint8_t>::iterator it = allteams.begin(); it != allteams.end(); it++){
		...
		for (SObjI so = this->getPos().grid->getObjInGrid().begin(); so != this->getPos().grid->getObjInGrid().end();so++){
			if (so->second->isShip() && so->second->getTeam() == (*it)){
				//we are invisible
				if (Rangeobj(so->second->getPos(),this->getPos()) < so->second->isShip()->getScanRange()/100){
					found = true;
...
void SShip::Move(uint32_t deltaT){
...
void SShip::MovePos(int32_t x, int32_t y){
	this->_pos.x+= x;
	this->_pos.y+= y;
}

void SShip::setTargetPos(SPos& pos){
	pthread_mutex_lock(&this->lockUnit);
	this->_targetPos.x = pos.x;
	this->_targetPos.y = pos.y;
	this->_targetPos.d = pos.d;
	...
	pthread_mutex_unlock(&this->lockUnit);
	this->getPos().grid->SendShipPosUpdate(this);
}

void SShip::setTargetPos(int32_t x, int32_t y, int32_t d){
	pthread_mutex_lock(&this->lockUnit);
	if(this->getPos().grid
...
uint32_t SShip::FitAddSub(SItemType* type, uint32_t slot, uint32_t Xitem, SCargoBay* cargobay){
	return cargobay->RemoveReturn(type ,this->AddSub(type,slot,cargobay->GetReturn(type,Xitem)));
}

uint32_t SShip::FitRemoveSub(uint32_t slot, uint32_t Xitem, SCargoBay* cargobay){
...
uint32_t SShip::AddSub(SItemType* type, uint32_t slot, uint32_t Xitem){
...
uint32_t SShip::RemoveSub(uint32_t slot, uint32_t Xitem){
	if (this->slots[slot]->getSS()){
...
void SShip::Hit(SShot* shot, uint32_t dmg, DmgTypes::Enum dmgtype, Shields::Enum impact, int32_t x, int32_t y){
...
void SShip::announceRemovalOf(SObj* obj){
